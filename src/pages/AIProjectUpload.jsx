...
function AIProjectUpload() {
  ...
  const handleProjectSubmit = (e) => {
    ...
    const projectsRef = ref(db, `users/${userId}/aiProjects`);

    push(projectsRef, project)
      .then(() => {
        setSuccess('AI project uploaded successfully!');
      ...
      .catch((err) => {
        console.error('Error uploading AI project:', err);
        setError('Failed to upload AI project');
        setTimeout(() => {
          setError('');
        }, 3000);
      });
  ...
  return (
    <form onSubmit={handleProjectSubmit} style={{ maxWidth: '600px', color: 'white', margin: '0 auto', padding: '20px', borderRadius: '8px', backgroundColor: '#444' }}>
      <h2 style={{ textAlign: 'center' }}>Upload AI Project</h2>
...
export default AIProjectUpload;
