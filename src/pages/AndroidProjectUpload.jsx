...
function AndroidProjectUpload() {
  ...
  const handleProjectSubmit = (e) => {
    ...
    const projectsRef = ref(db, `users/${userId}/androidProjects`);

    push(projectsRef, project)
      .then(() => {
        setSuccess('Android project uploaded successfully!');
      ...
      .catch((err) => {
        console.error('Error uploading Android project:', err);
        setError('Failed to upload Android project');
        setTimeout(() => {
          setError('');
        }, 3000);
      });
  ...
  return (
    <form onSubmit={handleProjectSubmit} style={{ maxWidth: '600px', color: 'white', margin: '0 auto', padding: '20px', borderRadius: '8px', backgroundColor: '#444' }}>
      <h2 style={{ textAlign: 'center' }}>Upload Android Project</h2>
...
export default AndroidProjectUpload;
